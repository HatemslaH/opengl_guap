...
use crate::ecs::components::{Camera, CameraLookTarget, Position, RenderMesh, SpinAnimation};
use crate::graphics::{
    MeshTopology, ShaderProgram, camera_view_projection_matrix,
    camera_yaw_pitch_deg_from_look_direction, model_matrix, view_projection_matrix,
};
use cgmath::Vector3;
use hecs::World;
...
pub fn render_mesh_system(world: &mut World, shader: &ShaderProgram, aspect: f32) {
    let vp = if let Some((_, (transform, cam))) = (&mut world.query::<(&Position, &Camera)>())
        .into_iter()
        .next()
    {
